...
namespace VidShare.Service
{
    public class UserService : IUserService
    {
        ...
        // ✔️ משאירים את השם כמו שהוא – אבל מסנכרנים אימות
        public async Task<User?> GetByLoginAsync(string name, string email)
        {
            // מכיוון שב-Repository קיימת הפונקציה עם password,
            // נחשיב email בתור password כדי לשמור תאימות
            return await _irepositoryManager.Users.GetByLoginAsync(name, email);
        }
    }
}
