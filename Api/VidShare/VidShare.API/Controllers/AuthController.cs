...
namespace VidShare.API.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        {
            // משתמש אחד אם נכנסים עם אימייל (כי יש @)
            User? user = null;

            if (loginModel.Name.Contains("@"))
            {
                // חיפוש לפי email
                user = await _userService.GetByLoginAsync(loginModel.Name, loginModel.Name);
            }
            else
            {
                // חיפוש רגיל לפי name + password
                user = await _userService.GetByLoginAsync(loginModel.Name, loginModel.Password);
            }
        ...
        {
            // בדיקה אם המשתמש כבר קיים לפי Email
            var existing = await _userService.GetByLoginAsync(dto.Name, dto.Email);
